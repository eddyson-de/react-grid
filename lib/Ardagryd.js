...
const Ardagryd = (props)=>{
        ...
        //filter function
        let filterFunction = config.filterFunction;
        ...
        let objects = props.objects.filter((currentObjectToBeFiltered) => {
            for (let i in columnNamesWithFilter){
                ...
                if (!currentObjectToBeFiltered[columnNamesWithFilter[i]]){
                    ...
                    if (columnConfig
                        && columnConfig[columnNamesWithFilter[i]]
                        && columnConfig[columnNamesWithFilter[i]].filterFunction) {
                        customFilterFunction = columnConfig[columnNamesWithFilter[i]].filterFunction;
                    }
                    if (customFilterFunction){
                        return customFilterFunction({value: currentObjectToBeFiltered[columnNamesWithFilter[i]],
                            expression: filters[columnNamesWithFilter[i]]});
                    } else {
                        return filterFunction({value: currentObjectToBeFiltered[columnNamesWithFilter[i]],
                                                expression: filters[columnNamesWithFilter[i]]});
                    }
                }
            }
    ...
    if (columnName){
            ...
            let mapped = objects.map(function(el, i) {
              ...
              let value = sortValueGetter({value: el[columnName], object: el});
...
const defaultConfig = {
    ...
    sortValueGetter: ({value}) => value && typeof value == "string" ? value.toLowerCase() : value,
