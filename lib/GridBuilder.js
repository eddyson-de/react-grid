...
const DefaultColumns = GridComponent => {
    return class DefaultColumnsWrapper extends React.Component {
        render(){
          ...
          if (columnConfigs && columnConfigs.length && objects && objects.length){
          ...
          return(
              <GridComponent columnConfig={withUnconfiguredColumns} objects={objects} {...rest}/>
          );
        }
    }
};
...
class GridBuilderClass extends React.Component {
    render(){
...
GridBuilderClass.displayName = "GridBuilder";
export const GridBuilder = ConfigCollector(GridBuilderClass);
