...
if (process.env.NODE_ENV !== 'production'){
    Column.propTypes = {
        ...
        hideTools: (props, propName, componentName)=>{
            if (props[propName]){
                const columnName = props.name;
                return new Error(
                    `Invalid prop \`${propName}\` supplied to \`${componentName}\` "${columnName}". Please use <Filter hide />.`);
            }
        },
        filter: (props, propName, componentName)=>{
            if (props[propName]){
                const columnName = props.name;
                return new Error(
                    `Invalid prop \`${propName}\` supplied to \`${componentName}\` "${columnName}. Please use the \`filter\` prop on the root component.`
                );
            }
        },
        sort: (props, propName, componentName)=>{
            if (props[propName]){
                const columnName = props.name;
                return new Error(
                    `Invalid prop \`${propName}\` supplied to \`${componentName}\` "${columnName}. Please use the \`sort\` prop on the root component.`
                );
            }
        },
        order: (props, propName, componentName)=>{
            if (props[propName]){
                const columnName = props.name;
                return new Error(
                    `Invalid prop \`${propName}\` supplied to \`${componentName}\` "${columnName}. The order is determined by the order of the \`${componentName}\` components.`
                );
            }
        },
        filterFunction:  (props, propName, componentName)=>{
            if (props[propName]){
                const columnName = props.name;
                return new Error(
                    `Invalid prop \`${propName}\` supplied to' \`${componentName}\` "${columnName}. Please use the \`Filter\` component with an appropriate \`match\` prop.`
                );
            }
        },
    };
