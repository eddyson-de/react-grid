        ...
        const cells = props.columnKeys.map((key) => {
            ...
            if(configForColumn && configForColumn.cellRenderer){
                ...
                if (value == null){
                    ...
                    return <Cell key={key} columnName={key} content={value} />;
                }
                else if (typeof value === 'string' || typeof value === 'number' || typeof value === 'boolean'){
                    return <Cell key={key} columnName={key} content={value.toString()} />;
                }
            }

            return (
                <Cell key={key} columnName={key} content={<CellRenderer config={props.config} value={value} columns={props.columns} columnName={key} object={current}/>} />
            );
        });
