...
const GridBody = ({config, objects, idColumn, columns, columnKeys, children})=>{
    const rows = objects.map((current) => {
        ...
        const component = config.row ? config.row.component : undefined;
        return(
            <GridRow key={current[idColumn]} columns={columns} config={config} object={current} component={component}>
                {cells}
            </GridRow>
        );
    });
